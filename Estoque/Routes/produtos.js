...
router.get('/', async (req, res) => {
    try {
        // CORRIGIDO: Usando a tabela PRODUTO (assumindo que o erro estava nas outras rotas)
        const result = await db.query('SELECT * FROM PRODUTO');
        const rows = result[0]
        res.render('produtos/lista', { produtos: rows });
    } catch (err) {
        console.error(err);
...
router.post('/criar', async (req, res) => {
    ...
    try {
        ...
        await db.query(
            'INSERT INTO PRODUTO (nome_produto, descricao, quant_estoque, estoque_minimo, data_cadastro, valor, ativo, id_categoria) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
...
router.get('/editar/:id', async (req, res) => {
    ...
    try {
        ...
        if (rows.length > 0) {
            res.render('produtos/editar', { produto: rows[0] });
        } else {
...
router.post('/editar/:id', async (req, res) => {
    ...
    try {
        // CORRIGIDO: Trocado 'produtos' por 'PRODUTO'
        await db.query(
            'UPDATE PRODUTO SET nome_produto = ?, descricao = ?, quant_estoque = ?, estoque_minimo = ?, valor = ?, ativo = ?, id_categoria = ? WHERE id_produto = ?',
